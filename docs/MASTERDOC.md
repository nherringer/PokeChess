...
- **Persona system:** `bot/persona.py` defines a `Persona` dataclass and six instances. Each exposes `to_bot_params()` which produces the dict stored in `bots.params`. The engine's `PersonaParams` model reads `time_budget`, `exploration_c`, `use_transposition`, `move_bias`, and `bias_bonus`. A **UCB1 bias bonus** is added to matching child nodes on every selection pass for `chase_pikachu` (Team Rocket) and `prefer_pikachu_raichu` (Clemont) — see [bot_personas.md](bot_personas.md).
